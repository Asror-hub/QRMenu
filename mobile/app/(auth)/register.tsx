...
import { normalizePhoneToE164 } from "@/src/utils/phoneAuth";

const GRAY_WHITE = "#D8D4D5";
const GRAY_WHITE_DARK = "rgba(255, 255, 255, 0.28)";

type FieldName = "restaurant" | "email" | "phone" | "password" | "confirm";

async function ensureOwnerRestaurant(opts: {
  userId: string;
  name: string;
  email: string;
  phone?: string | null;
...
export default function Register() {
  ...
  const handleSubmit = async () => {
    ...
    if (!name) {
      setLoading(false);
      setError(t("authErrorRestaurantName"));
      return;
    }
    ...
    if (typedPhone) {
      ...
      if (!contactPhone) {
        setLoading(false);
        setError(t("authErrorValidPhone"));
        return;
      }
    }
    ...
    if (password.length < 6) {
      setLoading(false);
      setError(t("settingsAccountErrMinLength"));
      return;
    }
    if (password !== confirmPassword) {
      setLoading(false);
      setError(t("settingsAccountErrMismatch"));
      return;
    }
    ...
    if (signUpError) {
      setLoading(false);
      setError(signUpError.message || t("authErrorSignupFailed"));
      return;
    }
    ...
    if (!session) {
      ...
      if (signInError || !signInData.session) {
        setLoading(false);
        setError(t("authErrorConfirmEmail"));
        return;
      }
    ...
    if (!created) {
      ...
      setError(t("authErrorSignupFailed"));
    }
  ...
  const fieldBorder = (empty: boolean) => {
    if (error && empty) return colors.danger;
    return borderColor;
  };
      ...
      >
        ...
        >
          ...
          >
            ...
            <Field>
              <Label style={{ color: labelColor }}>{t("authRestaurantName")}</Label>
              <InputWrap
                style={{
                  backgroundColor: fieldFill,
                  borderColor: fieldBorder(!restaurantName.trim()),
                }}
              >
                <Ionicons
                  name="storefront-outline"
                  size={20}
                  color={focused === "restaurant" ? colors.primary : colors.textMuted}
                />
                <Input
                  style={{ color: colors.text }}
                  value={restaurantName}
                  onChangeText={setRestaurantName}
                  placeholder={t("authRestaurantPlaceholder")}
            ...
            <Field>
              ...
              <InputWrap
                style={{
                  backgroundColor: fieldFill,
                  borderColor: fieldBorder(!email.trim()),
                }}
              >
                <Ionicons
                  name="mail-outline"
            ...
            <Field>
              ...
              >
                <Ionicons
                  name="call-outline"
                  size={20}
                  color={focused === "phone" ? colors.primary : colors.textMuted}
                />
                <Input
                  ref={phoneRef}
                  style={{ color: colors.text }}
            ...
            <Field>
              ...
              <InputWrap
                style={{
                  backgroundColor: fieldFill,
                  borderColor: fieldBorder(!confirmPassword),
                }}
              >
                <Ionicons
                  name="lock-closed-outline"
                  size={20}
                  color={focused === "confirm" ? colors.primary : colors.textMuted}
                />
                <Input
                  ref={confirmRef}
                  style={{ color: colors.text }}
            ...
            >
              {loading ? (
                ...
                <SubmitText>{t("authRegister")}</SubmitText>
              )}
            </Submit>
            <GetStarted>
              ...
              <Pressable
                ...
                accessibilityRole="button"
                accessibilityLabel={t("authSignIn")}
              >
